...
class ConfigStep(models.Model):
    ...
    def _run_step(self, build, **kwargs):
        ...
        if docker_params:
            if 'cpu_limit' not in docker_params:
                max_timeout = self.env['ir.config_parameter'].get_int('runbot.runbot_timeout', 10000)
            ...
            container_cpus = self.container_cpus or self.env['ir.config_parameter'].sudo().get_float('runbot.runbot_containers_cpus')
    ...
    def _run_install_odoo(self, build, config_data=None):
        ...
        if "--screencasts" in available_options and (self.env['ir.config_parameter'].sudo().get_bool('runbot.enable_screencast') or config_data.get('screencast', False)):
