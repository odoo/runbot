                ...
                for db in valid_databases:
                    ...
                    if self.allow_similar_build_quick_result:
                        ...
                        if not existing_done_build and not build.create_batch_id.bundle_id.is_staging:
                            existing_done_build = next((build for build in child.params_id.build_ids.sorted('id') if build.global_state == 'done' and build.local_result not in ('skipped', 'killed') and not build.make_orphan), None)
