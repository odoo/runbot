...
class Host(models.Model):
    ...
    nb_worker = fields.Integer(
        ...
        default=lambda self: self.env['ir.config_parameter'].sudo().get_int('runbot.runbot_workers', default=2),
        tracking=True,
    )
    ...
    def _static_url(self):
        use_ssl = self.env['ir.config_parameter'].get_bool('runbot.use_ssl', default=True)
    ...
    def _get_docker_registry_url(self):
        ...
        icp = self.env['ir.config_parameter']
        docker_registry_url = icp.get_str('runbot.docker_registry_url', default=None)
        if docker_registry_url:
            return docker_registry_url.strip('/')
        docker_registry_host = self.browse(int(icp.get_int('runbot.docker_registry_host_id', default=0)))
    ...
    def _get_running_max(self):
        icp = self.env['ir.config_parameter']
        return self.nb_run_slot or int(icp.get_int('runbot.runbot_running_max', default=5))
