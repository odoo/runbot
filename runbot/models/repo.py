...
class Repo(models.Model):
    ...
    def _git(self, cmd, errors='strict', quiet=False, input_data=None, raw=False):
        ...
        fetch_timeout = self.env['ir.config_parameter'].get_int('runbot.runbot_fetch_timeout', 30)
    ...
    def _update_batches(self, force=False, ignore=None):
        ...
        for repo in self:
            if repo.remote_ids and self._update(poll_delay=30 if force else 60*5):
                max_age = self.env['ir.config_parameter'].get_int('runbot.runbot_max_age', 30)
