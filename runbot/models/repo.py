...
class runbot_repo(models.Model):
    ...
    repo_run_config_id = fields.Many2one('runbot.build.config', 'Run Config')
    ...
    def _compute_run_config_id(self):
        for repo in self:
            if repo.repo_run_config_id:
                repo.run_config_id = repo.repo_run_config_id
            else:
                repo.run_config_id = self.env.ref('runbot.runbot_build_config_default')
    
    def _inverse_run_config_id(self):
        for repo in self:
            repo.repo_run_config_id = repo.run_config_id
