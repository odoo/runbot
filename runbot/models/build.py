...
class BuildResult(models.Model):
    ...
    def _compute_gc_date(self):
        ...
        max_days_main = icp.get_int('runbot.db_gc_days', 30)
        max_days_child = icp.get_int('runbot.db_gc_days_child', 15)
        for build in self:
    ...
    def _get_run_url(self, db_suffix=None):
        ...
        use_ssl = self.env['ir.config_parameter'].sudo().get_bool('runbot.use_ssl', True)
    ...
    def _filter_to_clean(self, dest_list, label):
        ...
        hide_in_logs = icp.get_str('runbot.runbot_db_template', 'template0')
    ...
    def _local_cleanup(self, force=False, full=False):
        ...
        icp = self.env['ir.config_parameter']
        full_gc_days = icp.get_int('runbot.full_gc_days', 365)
    ...
    def _find_port(self):
        ...
        # starting port
        icp = self.env['ir.config_parameter']
        port = icp.get_int('runbot.runbot_starting_port', 2000)
    ...
    def _schedule(self):
        ...
        if build.local_state == 'pending':
            ...
            if _docker_state == 'RUNNING':
                ...
                timeout = min(build_limit, icp.get_int('runbot.runbot_timeout', 10000))
    ...
    def _docker_run(self, step, cmd=None, ro_volumes=None, env_variables=None, **kwargs):
        ...
        containers_memory_limit = self.env['ir.config_parameter'].sudo().get_float('runbot.runbot_containers_memory')
        if containers_memory_limit and 'memory' not in kwargs:
            ...
            containers_memory_limit = int(containers_memory_limit * 1024 ** 3) * memory_limit_factor
